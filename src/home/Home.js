...
import "./home.css";
import logo from "../ScreenSpeakLogo.webp";

function Home() {
  // useStates for displayed information to be set from mongodb fetch calls
  ...
  const [reviewsPerPage] = useState(10); // Assuming you want 10 reviews per page
  ...
  const currentReviewsUser = filteredReviewsForUser.slice(
    indexOfFirstReviewUser,
    indexOfLastReviewUser
  );
  ...
  // fetch users and display information from db
  ...
  return (
    <div className="px-2 bg-main">
      ...
      <div className="logo-section">
        <img src={logo} alt="ScreenSpeak Logo" className="logo-image" />
      </div>
      {/* Welcome message depending on if signed in */}
      <div className="welcome-message">
        <h1>{currentUser ? `Welcome, ${currentUser.name}!` : "Welcome to ScreenSpeak!"}</h1>
        {!currentUser && (
          <p>Sign up or log in to start exploring movies and reviews from other users.</p>
        )}
      </div>
      <div>
        <div className="row" style={{ width: "100%" }}>
              ...
              {currentUser
                ? //Filter all the following users for the activity feed
                  currentReviewsUser.map((review, index) => (
                    <Link key={index} to={`/details/${review.movie_id}`} state={{ from: `/` }}>
          ...
          <div className="col-xl-4 col-lg-6 col-md-6 col-sm-12 col-xs-12"></div>
        </div>
      </div>
    </div>
  );
